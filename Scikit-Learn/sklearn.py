...
# Q3.4.3 Hyper-parameter Tuning
    ...
    def SVMBestScore(self, scaled_x_train, y_train):
        ...
        # ADD CODE HERE
        svc = SVC(gamma='auto')
        svm_cv = GridSearchCV(estimator = svc, param_grid = svm_parameters, n_jobs = -1, return_train_score = True)
    ...
    def SVCClassifierParam(self,svm_cv,scaled_x_train,scaled_x_test,y_train):
        ...
        # -------------------------------
        # ADD CODE HERE
        svc_param = SVC(C = svm_cv.best_params_['C'], kernel = svm_cv.best_params_['kernel'])
        svc_param.fit(scaled_x_train, y_train)
        y_predict_train = svc_param.predict(scaled_x_train)
        y_predict_test = svc_param.predict(scaled_x_test)
        # -------------------------------
        return y_predict_train,y_predict_test
